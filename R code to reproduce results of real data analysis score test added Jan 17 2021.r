rm(list=ls());gc()
wd = 'C:/Users/gz085/Downloads/OneDrive - Partners HealthCare/BWH/NLME censoring manuscript/Datasets R code for Real Data Example/'
setwd(wd)
library(nlme)
library(ggplot2)
library(data.table) 
...
lmec_Amat_bvec_constraint = function(yL, cens, X, Z, cluster, maxstep = 200,init,epsstop = 0.05, abspmv = 0.001, mcmc0 = 100, sdl = 0.1, iter2 = 15, trs = 5, pls = 5, mcmcmax = 1000, Amat,bvec,meq=0)
{
  ...
  #### the score test requires the 1st order partial derivatives of LME log-likelihood with respect to beta, which is given by sum( Xi^T %*% solve(Vi)%*%ri )/sigma2, where Vi= (I+Zi%*%(Psi/sigma2)%*%Zi^T),ri=yl-Xi%*%beta, as well as the 2nd order partial derivatives of LME log-likelihood with respect to beta, which is given by  sum( Xi^T %*% solve(Vi)%*%Xi )/sigma2,  # see Lindstrom, M.J. and Bates, D.M., 1988. Newton-Raphson and EM algorithms for linear mixed-effects models for repeated-measures data. Journal of the American Statistical Association, 83(404), pp.1014-1022.
  D = Psi/sigma2
  sum_Xi_T_solve_Vi_ri=rep(0,p)
  sum_Xi_T_solve_Vi_Xi=diag(rep(0,p))
  residual_at_convergence = rep(0,length(y))
  for (i in 1:m)
  {  
    Xi = X[cluster==i, , drop = FALSE] 
    Zi = Xi[, bi.subset,drop=F]
    solve_Vi = solve( diag(ni[i])+Zi%*%D%*%t(Zi) ) 
    ri = y[cluster==i] -Xi%*%beta
    residual_at_convergence[cluster==i]=ri
    sum_Xi_T_solve_Vi_ri = sum_Xi_T_solve_Vi_ri + t(Xi)%*%solve_Vi%*%ri
    sum_Xi_T_solve_Vi_Xi = sum_Xi_T_solve_Vi_Xi+ t(Xi)%*%solve_Vi%*%Xi
  }
 
  return(list(beta = beta, bi = b, sigma = sqrt(sigma2), 
              Psi = Psi, Delta = Delta, loglik = lik, loglikex = likex, 
              varFix = varFix, 
              step = iter, likseq = likseq[1:iter], likseqex = likseqex[1:iter],residual_at_convergence=residual_at_convergence,sum_Xi_T_solve_Vi_ri=sum_Xi_T_solve_Vi_ri,sum_Xi_T_solve_Vi_Xi=sum_Xi_T_solve_Vi_Xi))
}
...
# score test 
score_vector_at_theta_tilde = lme_out315$sum_Xi_T_solve_Vi_ri[c(2,5)]/lme_out315$sigma^2 
score_vector_at_theta_bar = lme_out315H0$sum_Xi_T_solve_Vi_ri[c(2,5)]/lme_out315H0$sigma^2  
Tscore_315 = (t(score_vector_at_theta_bar-score_vector_at_theta_tilde)%*%solve(lme_out315$sum_Xi_T_solve_Vi_Xi[c(2,5),c(2,5)])%*%(score_vector_at_theta_bar-score_vector_at_theta_tilde))[1,1]
Tscore_315
Tscore_pval315 = 1-q-0.5*pchisq(Tscore_315,df=1)-(0.5-q)*pchisq(Tscore_315,df=2) 
Tscore_pval315
...
# score test 
score_vector_at_theta_tilde = lme_out398$sum_Xi_T_solve_Vi_ri[c(2,4)]/lme_out398$sigma^2 
score_vector_at_theta_bar = lme_out398H0$sum_Xi_T_solve_Vi_ri[c(2,4)]/lme_out398H0$sigma^2  
Tscore_398 = (t(score_vector_at_theta_bar-score_vector_at_theta_tilde)%*%solve(lme_out398H0$sum_Xi_T_solve_Vi_Xi[c(2,4),c(2,4)])%*%(score_vector_at_theta_bar-score_vector_at_theta_tilde))[1,1]
Tscore_398
Tscore_pval398 = 1-q-0.5*pchisq(Tscore_398,df=1)-(0.5-q)*pchisq(Tscore_398,df=2) 
Tscore_pval398
